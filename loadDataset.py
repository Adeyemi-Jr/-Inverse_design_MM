...
def getDataset():
    ...
    val_split_length = round(len(dataset) * valSplit)

    split_ratio = [train_split_length,test_split_length,val_split_length]
    print('train/test/validation: ', split_ratio )
    train_set, test_set, val_set = torch.utils.data.random_split(dataset, split_ratio , generator=torch.Generator().manual_seed(42))
    return
